...
class ElectionResultValidator(SQLModel):
    ...
    updated_at: ClassVar[datetime] = SQLModelField(
        sa_column=Column(
            TIMESTAMP(timezone=True),
            server_default=text("CURRENT_TIMESTAMP"),
...
class CandidateRaceLink(SQLModel, table=True):
    candidate_id: int = SQLModelField(foreign_key='candidate.full_name', primary_key=True)
    race_id: int = SQLModelField(foreign_key="racedetails.id", primary_key=True)


class RaceCountyLink(SQLModel, table=True):
    county_id: int = SQLModelField(foreign_key='county.name', primary_key=True)
    race_id: int = SQLModelField(foreign_key="racedetails.id", primary_key=True)
...
class StatewideCanadidateRaceLink(SQLModel, table=True):
    candidate_id: int = SQLModelField(foreign_key='candidate.full_name', primary_key=True)
    statewide_candidate_id: int = SQLModelField(foreign_key='statewidecandidatesummary.name', primary_key=True)
